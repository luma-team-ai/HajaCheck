...
9. 새 애플리케이션을 배포하고 Hibernate `ddl-auto=validate` 통과와 애플리케이션 헬스를 확인한다.
...
`ApiSystemLogSchemaMigrationTest`는 v0.3 기준선에 `20260720_01_create_api_system_logs.sql`을 2회 적용해
재실행 가능성을 확인하고, identity·기본값·CHECK·인덱스 3개·`user_id` FK 부재와 `request_id` 비유니크
정책을 검증한다. 같은 이름의 CHECK 또는 인덱스를 다른 의미로 바꾼 뒤 재실행하면 마이그레이션이 즉시
중단되는지도 확인한다. `Ha25IncrementalMigrationTest`도 이 독립 파일을 전체 증분 경로에 포함한 뒤
캐노니컬 DDL 신규 설치 DB와 PostgreSQL 카탈로그를 대조한다.
...
동일 이름 테이블·제약·인덱스가 이미 있으면 캐노니컬 DDL과 컬럼 타입·NULL·기본값·identity·CHECK·PK·
인덱스 의미가 정확히 일치할 때만 재실행을 허용하며, `user_id` FK를 포함한 의미 드리프트가 있으면 즉시
중단한다. 신규 빈 테이블이라 `CREATE INDEX CONCURRENTLY`는 사용하지 않는다.
