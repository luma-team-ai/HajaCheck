...
10. `20260722_01_defects_add_media.sql`을 실행해 `defects.media_id` 컬럼을 추가한다.
11. 새 애플리케이션을 배포하고 Hibernate `ddl-auto=validate` 통과와 애플리케이션 헬스를 확인한다.
...
`20260722_01_defects_add_media.sql`도 독립 파일이다(#527 / HAJA-314 하자 상세 화면 실사진 표시) —
`defects.media_id` nullable 컬럼과 `idx_defects_media` 인덱스만 추가한다. AI 탐지 파이프라인이 아직
없어 신규 Defect 생성 경로 자체가 프로덕션 코드에 없으므로 백필 대상 데이터가 없고, 기존 행은 전부
NULL로 남는다. 신규 설치는 `HajaCheck_script.sql`에 이미 반영된 동일 컬럼을 사용하므로 이 파일이 필요
없고, 기존 운영 DB만 단독으로 적용한다.
