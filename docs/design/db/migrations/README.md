...
`Ha25IncrementalMigrationTest`는 별도 PostgreSQL 17에서 v0.3 기준 DDL → expand 2회 → finalize 2회 →
verify를 실행한 뒤 Hibernate `ddl-auto=validate`로 전체 Entity 매핑을 대조한다. 따라서 Flyway 없이도
캐노니컬 신규 설치 경로와 수동 증분 경로의 스키마 드리프트를 기본 Gradle 테스트에서 탐지한다.
