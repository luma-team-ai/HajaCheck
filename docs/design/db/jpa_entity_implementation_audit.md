...
HAJA-25 착수 시점 통합 DDL에는 PostgreSQL named enum 23개와 테이블 20개가 정의되어 있었고,
현재 백엔드에는 그중 9개 테이블에 대응하는 JPA Entity가 구현되어 있었다. 이후 별도 메뉴 스키마 작업으로
`menu_node_type`, `menus`, `menu_role_access`가 기준 DDL에 추가됐으며, 이 2개 테이블은 HAJA-25 Entity
구현 범위에 포함하지 않는다.
...
- 현재 Entity 구현 수: 20개(HAJA-25 대상 DDL 테이블 20개 전체 대응, 후속 메뉴 테이블 2개 제외)
...
스크립트 3개와 메뉴 증분 스크립트 2개를 검사해 classpath로 직접 복사하고, 파일이 하나라도 없으면
테스트 자원 처리 전에 빌드를 중단한다. Repository 테스트는 캐노니컬 DDL로 초기화하며 전체 Entity를
`ddl-auto=validate`로 대조한다.
...
모두 운영 목표와 같은 `postgres:16`으로 통일했다.

`MenuSchemaMigrationTest`는 v0.3 기준선에 메뉴 expand를 두 번 적용하고, CHECK·FK 삭제 정책·역할 매핑
CASCADE·`updated_at` 트리거를 검증한다. 이후 GROUP 직접 역할 매핑과 같은 이름의 잘못된 FK·인덱스를
주입해 메뉴 verify가 의미 드리프트를 거부하는지도 확인한다.
...
필요하다. Repository 및 증분 테스트의 Testcontainers 기본 이미지도 운영 목표와 같은 `postgres:16`으로
통일했다.
