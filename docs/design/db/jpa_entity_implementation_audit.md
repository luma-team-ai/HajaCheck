...
- Spring Boot 컨벤션은 `auth`, `core`, `counsel`, `admin` 사이의 Entity 직접 참조를 금지한다.
- 현재 Entity는 이 경계를 지키기 위해 외래키를 주로 `Long` 값으로 보유한다.
- HAJA-199~204는 필요한 객체 연관관계 정의를 요구한다.

따라서 같은 도메인 내부 관계만 객체로 매핑하고 도메인 경계를 넘는 FK는 ID로 유지할지,
DDL의 모든 FK를 객체 연관관계로 매핑할지 결정해야 한다. 결정 전 일괄적인 `Long` FK 전환은 하지 않는다.
...
- 전체: 129개
- 실패: 49개
- 실패 원인: 로컬 환경에 Docker CLI/엔진이 없어 Testcontainers의 PostgreSQL 컨테이너 시작 실패
- 컴파일 및 Docker 비의존 테스트는 실행 가능

Docker가 준비된 환경에서 동일 명령을 다시 실행해야 현재 코드의 실제 PostgreSQL 테스트 기준선을 확정할 수 있다.
