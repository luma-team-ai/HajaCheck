...
comment on column users.company_id is '소속된 기업 계정 식별자(companies). 개인 사용자는 NULL이며, 가입 이후에도 기업에 소속될 수 있다.';
...
comment on column user_consents.user_id is '동의한 사용자 식별자. 감사·분쟁 대응을 위해 동의 이력은 사용자 삭제와 독립적으로 보존하며, 물리 삭제는 ON DELETE RESTRICT로 차단한다(사용자 탈퇴는 soft delete로 처리).';
...
comment on table user_plans is '개인(user_id) 또는 회사(company_id)에 적용된 구독 요금제와 이용 기간을 관리한다. 회사 귀속 행은 소속 임직원 전체의 플랜 게이팅을 대표한다.';
