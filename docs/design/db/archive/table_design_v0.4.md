...
5. [테이블 상세](#5-테이블-상세)
   ...
   - 5.6 [공통 알림](#56-공통-알림)
   - 5.7 [사이드바·관리자 메뉴](#57-사이드바관리자-메뉴)
   - 5.8 [API 시스템 로그](#58-api-시스템-로그)
...
착수 보고서(`hajaCheck착수 보고.pdf`) 53~60p "핵심 테이블 정의서 1/8~8/8"에는 8개 핵심 테이블(`users`, `facilities`, `inspections`, `reports`, `chat_sessions`, `rag_documents`, `plans`, `notifications`)만 정의되어 있었다. 이후 기준 DDL(`HajaCheck_script.sql`) 작업 과정에서 세부 설계가 구체화되면서 아래와 같이 달라졌다.

### 2.1 기존 8개 핵심 테이블의 변경 내역

| 테이블 | 착수 보고서(53~60p) 시점 | 최종 `HajaCheck_script.sql` | 변경 취지 |
|---|---|---|---|
| `users` | `social_provider`, `social_id` NULL 허용 | 두 컬럼 NULL 허용 유지 + `password_hash`, `company_id` 추가 + `unique(social_provider, social_id)` + `ck_users_auth_method` | 소셜 로그인과 이메일/비밀번호 자체가입을 함께 지원하면서, 동일 소셜 계정 중복 가입을 방지하고 최소 하나의 인증 수단을 강제 |
...
| `rag_documents` | `id, title, source_type, file_url, embedding_status, chunk_count, embedded_at, created_at` | `target_collection`, `effective_date`, `publisher`, `authored_at`, `verification_status` 추가 | PostgreSQL은 계속 문서 메타데이터만 관리하되, Chroma 컬렉션 라우팅·법규 시행일·발행처·작성일·검증 상태를 추적하도록 확장(§2.7, §2.8) |
...
### 2.9 사이드바·관리자 메뉴 관리 테이블 신설 (`menus`/`menu_role_access`)

**배경**: 사이드바·관리자 메뉴 구성을 코드에 하드코딩(`SideNavBar.tsx`의 `DEFAULT_ITEMS`/`DEFAULT_ADMIN_ITEM`)하는 대신 DB에서 관리하기 위해 신규 설계했다. 권한 매트릭스(§2.4.2 `role_type` 인용 근거, `PRD_hajaCheck.md` §역할별 권한 매트릭스)가 상담원에게는 "관리자 페이지 — 상담 관리만", 관리자에게는 "전체"를 요구하므로, 메뉴 노출을 역할 하나에 종속된 컬럼으로 표현할 수 없어 메뉴 노드(`menus`)와 역할별 접근(`menu_role_access`)을 **테이블 2개로 분리**했다.
...
### 2.10 API 시스템 로그 테이블 신설 (`api_system_logs`, #497/HAJA-299)

**배경**: 관리자 모니터링의 기반 데이터로, API 호출이 최종적으로 4xx 또는 5xx 응답을 반환한 경우의 요청
메타데이터를 저장한다. 애플리케이션 내부의 모든 Logback 로그나 스케줄러·배치 로그를 수집하는 범용 로그
저장소는 이번 범위가 아니다.

| 결정 항목 | 확정 내용 |
|---|---|
| 수집 범위 | API 최종 응답이 400~499이면 `WARN`, 500~599이면 `ERROR`로 기록한다. 1xx~3xx는 저장하지 않는다 |
| 기록 단위 | API 요청당 최대 한 행을 애플리케이션에서 보장한다. 동일 요청의 재처리·보정 가능성을 막지 않도록 `request_id`에 UNIQUE 제약은 두지 않는다 |
...
menus ──< menus (parent_id, self-referencing tree)
menus ──< menu_role_access >── role_type (N:M, 매핑 행 존재 = 해당 역할에 노출)
users ──< menus (created_by/updated_by, 변경 관리자 추적)
users ──< menu_role_access (created_by)

api_system_logs.user_id ···> users.id (논리 참조, FK 없음)
...
- `api_system_logs`는 API 요청의 최종 4xx/5xx 결과를 독립적으로 기록한다. `user_id`는 조회 보조용 논리
  참조라 `users`와 FK 관계를 만들지 않으며, `request_id`도 요청당 최대 한 행이라는 애플리케이션 정책을
  DB UNIQUE로 강제하지 않는다.
...
| `menu_node_type` | `GROUP`, `INTERNAL`, `EXTERNAL` | 사이드바 메뉴 노드 유형(그룹/내부 링크/외부 링크) | `menus.menu_type` |
...
- 인덱스: `idx_company_memberships_company_status (company_id, status)`, `idx_company_memberships_user_status (user_id, status)`
- 트리거: `trg_company_memberships_set_updated_at` — 행 수정 시 `updated_at` 자동 갱신
...
- 점검 담당자 배정도 요청자와 배정자의 `users.company_id` 일치만으로 허용하지 않는다. 두 사용자 모두 위 유효 멤버십 조건을 만족할 때만 같은 회사의 담당자로 인정한다.
...
- 신뢰 경계: Entity 팩토리는 호출자에게 `mime_signature_verified`를 받지 않고 신규 행을 항상 `false`로 생성한다. 업로드 서비스가 서버에서 실제 파일 바이트의 매직바이트와 MIME 타입 일치를 확인한 뒤에만 인자 없는 `markMimeSignatureVerified()`를 호출한다. 이 전이는 `false→true` 단방향이며, 소비 경로는 `true`인 행만 시그니처 검증 완료로 신뢰한다.
...
- **Grounding 신뢰 경계**: 콘텐츠 수정은 기존 grounding 판정을 `NULL`로 무효화한다. 판정값은 요청 DTO의
  ...
  의존하지 않으며, `finalizeReport`는 이 결과가 통과일 때만 허용한다.
...
- **부분 UQ** `uq_counsel_tickets_session`: non-null `session_id`는 한 티켓에서만 사용할 수 있다. 하나의 전문상담 세션이 여러 티켓에 중복 배정되는 것을 DB 레벨에서 차단한다.
...
- 인덱스: `idx_rag_documents_embedding_status (embedding_status)`, `idx_rag_documents_target_collection (target_collection)`
...
#### `menus` — 사이드바 및 관리자 메뉴 트리

| 컬럼 | 타입 | NULL | 기본값 | 키 | 설명 |
|---|---|---|---|---|---|
| id | bigint (identity) | N | - | **PK** | 메뉴 식별자 |
| code | varchar(100) | N | - | UQ | 변경되지 않는 고유 메뉴 코드(예: `DASHBOARD`, `ADMIN_USERS`) |
| name | varchar(100) | N | - | | 표시 메뉴명 |
| menu_type | menu_node_type | N | - | | 메뉴 노드 유형(`GROUP`/`INTERNAL`/`EXTERNAL`) |
| parent_id | bigint | Y | - | **FK→menus**(`fk_menus_parent`, ON DELETE RESTRICT) | 상위 메뉴 식별자 |
| path | varchar(500) | Y | - | | 이동 경로. GROUP은 NULL, 그 외는 필수. 중복 가능해 UQ 아님 |
| active_path_pattern | varchar(500) | Y | - | | 실제 라우트가 path와 다를 때 활성 메뉴 판정용 동적 경로 패턴 |
| icon_key | varchar(100) | Y | - | | 프론트 번들 아이콘 키 |
| icon_url | varchar(500) | Y | - | | CDN 아이콘 URL(icon_key와 동시에 채우지 않음) |
| sort_order | integer | N | 0 | | 동일 부모 하위 노출 순서 |
| is_visible | boolean | N | true | | 메뉴 표시 여부 |
| is_enabled | boolean | N | true | | 클릭 가능 여부(미구현 메뉴 등) |
| opens_new_tab | boolean | N | false | | 외부 링크 새 창 여부 |
| description | varchar(500) | Y | - | | 관리자용 메뉴 설명 |
| created_by | bigint | Y | - | **FK→users** | 생성한 관리자. 초기 시드는 NULL 허용 |
| updated_by | bigint | Y | - | **FK→users** | 마지막으로 수정한 관리자 |
| created_at | timestamptz | N | now() | | 생성 시각 |
| updated_at | timestamptz | N | now() | | 최종 수정 시각(트리거 자동 갱신) |

- **CK** `ck_menus_not_self_parent`: `parent_id`가 자기 자신을 가리킬 수 없음(직접 순환만 차단, 간접 순환 `A→B→A`는 서비스/재귀 CTE 트리거에서 별도 차단).
- **CK** `ck_menus_sort_order_nonnegative`: `sort_order >= 0`.
- **CK** `ck_menus_icon_single`: `GROUP`은 `icon_key`/`icon_url` 0~1개, 그 외는 정확히 1개.
- **CK** `ck_menus_path_by_type`: `GROUP`이면 `path IS NULL`, 그 외면 `path IS NOT NULL`.
- 인덱스: `idx_menus_parent (parent_id)`
- 트리거: `trg_menus_set_updated_at`
- `lock_version`을 두지 않는다 — 소수 관리자가 드물게 편집하는 설정 테이블이라 동시 갱신 충돌 위험이 낮다(§2.9).
- 정렬은 `ORDER BY sort_order, id`. 허용된 자식이 하나라도 있으면 부모 `GROUP`은 서비스 로직이 자동으로 트리에 포함시킨다.

#### `menu_role_access` — 메뉴별 역할 접근 매핑

| 컬럼 | 타입 | NULL | 기본값 | 키 | 설명 |
|---|---|---|---|---|---|
| menu_id | bigint | N | - | **PK(복합)**, **FK→menus**, ON DELETE CASCADE | 메뉴 식별자 |
| role | role_type | N | - | **PK(복합)** | 접근 가능한 역할 |
| created_by | bigint | Y | - | **FK→users** | 매핑을 등록한 관리자 |
| created_at | timestamptz | N | now() | | 매핑 등록 시각 |
...
- 메뉴 노출은 편의 기능일 뿐 보안 권한이 아니다. 관리자 API는 이 테이블과 별개로 `@PreAuthorize` + 소유권·회사 멤버십 검증을 반드시 병행한다(§2.9).

---

### 5.8 API 시스템 로그

#### `api_system_logs` — API 4xx/5xx 요청 시스템 로그
...
| `trg_company_memberships_set_updated_at` | 트리거 (BEFORE UPDATE, `company_memberships`) | `company_memberships` 행 수정 시 `updated_at` 자동 갱신 |
...
4. **운영 로그 최소 수집**: API 시스템 로그는 최종 4xx/5xx 결과에 필요한 메타데이터만 저장하고 요청·응답
   본문, 스택 트레이스, 인증정보 등 민감하거나 과도한 데이터는 저장하지 않는다.

---

## 9. 최종 산출물 목록

현재 DB 설계 기준 최종 산출물은 아래 파일들이다.
