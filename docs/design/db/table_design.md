...
| 영역 | 테이블 |
|---|---|
| 계정·인증 | `users`, `companies`, `user_consents` |
...
| 개인→기업 전환 | 가능해야 함 — `users.company_id`는 가입 시점이 아니라 **언제든 채워질 수 있는 nullable FK** |
...
| 접근 판단 로직(애플리케이션) | `has_counselor_access` 등 플랜 게이팅 항목은 **"내 개인 `user_plans` OR 내 `users.company_id`가 가리키는 회사의 `user_plans`"** 중 하나라도 ACTIVE + 해당 boolean이 true면 허용 |
...
companies ──< users (company_id, 개인→기업 전환 시 언제든 세팅 가능)
...
- `users`는 `facilities.owner_id`, `inspections.created_by`, `defect_revisions.revised_by`, `reports.created_by/edited_by`, `chat_sessions.user_id`, `counsel_tickets.user_id/counselor_id`, `notifications.user_id` 등 서비스 전반의 액터로 참조된다.
...
| company_id | bigint | Y | - | **FK→companies**(`fk_users_company`) | 소속 기업 계정. 개인 사용자는 NULL, 언제든 세팅 가능(개인→기업 전환) |
...
- 참조 대상: `facilities.owner_id`, `inspections.created_by`, `defect_revisions.revised_by`, `reports.edited_by/created_by`, `chat_sessions.user_id`, `counsel_tickets.user_id/counselor_id`, `notifications.user_id`, `user_plans.user_id`, `companies.owner_user_id/reviewed_by`, `user_consents.user_id`.
...
- 인덱스: `idx_inspections_facility (facility_id)`
...
|---|---|---|
| 계정·인증 | 소셜 로그인, 자체 회원가입, 개인·기업 가입, 관리자 승인 | `users`, `companies`, `user_consents`로 계정, 기업 승인, 약관 동의 이력 분리. `users`는 소셜 로그인과 이메일/비밀번호 가입을 모두 허용하도록 `ck_users_auth_method` 제약 적용 |
| 멤버십·쿼터 | Free/Standard/Enterprise 플랜, 월 분석 수, 시설 수, 좌석 수 제한, 기업 임직원의 회사 플랜 상속(§2.6) | `plans`, `user_plans`, `usage_counters`로 플랜 정책과 개인/회사(`user_plans.company_id`) 단위 월간 사용량을 분리. 애플리케이션의 `QuotaInterceptor`가 이 테이블을 기준으로 제한 판단 |
...
| 산출물 | 파일 | 용도 |
|---|---|---|
| 테이블 디자인 설계서 | `hajaCheck_테이블_디자인_설계.md` | 착수보고서·PRD 대비 변경 사항, ERD 개요, enum/테이블 상세, RAG·상담 공용 설계, 핵심 요구사항 요약을 설명하는 기준 문서 |
...
제출·공유 시에는 `hajaCheck_테이블_디자인_설계.md`를 설명 문서로, `hajaCheck_script.sql`을 최종 적용 기준으로 사용한다.