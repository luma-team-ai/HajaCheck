...
- 운영 중인 v0.3 계열 DB에는 이 파일을 그대로 재실행하지 않는다. HAJA-25 변경은
  [`migrations/README.md`](migrations/README.md)의 `expand → 백필 → finalize → verify` 증분 절차로 적용한다.
- Testcontainers는 Gradle `processTestResources` 단계에서 이 파일을 테스트 classpath로 복사해 사용한다.
  별도의 축약 DDL 사본은 유지하지 않는다.
