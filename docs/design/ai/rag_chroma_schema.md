...
class SourceCitation(BaseModel):
    ...
    snippet: str      # 검색된 Chroma 청크 원문 발췌 — locator와 별개
...
`chat_message_citations`(Postgres: `message_id`, `document_id`, `chunk_ref`, `locator`, `snippet`)와 필드를 맞췄다 — `SourceCitation.doc_id`는 양의 정수 문자열 검증 후 `int(doc_id)`로 변환해 `document_id`에 저장하고, `chunk_ref`→`chunk_ref`(그대로), `locator`→`locator`, `snippet`→`snippet`에 저장한다. `snippet`은 RAG 체인이 검색 단계에서 이미 확보한 해당 청크의 Chroma 원문 텍스트를 발췌한 값이며, `locator` 표시 문구를 복사한 값이 아니다(§7). Spring Boot는 FastAPI 내부의 검색 컨텍스트나 Chroma에 직접 접근하지 않으므로, 인용을 영속화하는 데 필요한 `snippet`을 `SourceCitation` API 응답으로 반드시 전달받는다. `collection`은 중복 저장하지 않고 이력 복원 시 불변인 `rag_documents.target_collection`을 조인해 `REGULATIONS`→`regulations`, `DEFECT_KB`→`defect_kb`로 변환한다. 문서를 다른 컬렉션으로 재분류할 때는 기존 행을 수정하지 않고 새 `rag_documents` 행을 생성해 재임베딩한다.
