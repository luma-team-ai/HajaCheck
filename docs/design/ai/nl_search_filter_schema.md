...
class NlSearchRequest(BaseModel):
    query: str  # 사용자가 입력한 자연어 질의 원문
```

- `query`가 빈 문자열/공백만 있는 경우는 **LLM 호출 이전에 코드로 검증**해 `VALIDATION_ERROR`로 즉시 실패시킨다(§5.3 빈 질의 케이스, LLM 호출 비용 방지).
- 현재 프론트의 수동 필터 상태는 요청에 포함하지 않는다 — 자연어 검색은 매 질의마다 **독립적으로 새 필터 조건을 만든다**(기존 수동 필터와의 병합 여부는 프론트가 응답을 받은 뒤 결정, HAJA-182 §4.2).
...
| 빈 질의 | (LLM 호출 안 함) | — | — | — → `VALIDATION_ERROR`로 실패 처리 |

- `clarifying_question`이 채워진 경우 프론트는 `filters`를 기존 상태에 적용하지 않고 질문만 노출한다(HAJA-182 §4.2).
...
- 빈 질의 검증은 체인 진입 전 라우터(`ai_router.py`)에서 처리 — `run_nl_search_chain`은 최소 1자 이상의 `query`만 받는다.
...
사용자는 confidenceMin을 퍼센트로 표현한다("80%", "80퍼센트", "신뢰도 80 이상" 등). **100으로 나눠 0~1 소수로 변환**해서 confidenceMin에 넣어라.
...
## 4. `/ai/nl-search` ↔ `/api/defects` ↔ 프론트 연동 계약 (HAJA-182)
...
- **결정**: 필터 값 표현은 **영문 DB enum 코드로 통일**한다. `/ai/nl-search` 응답, `GET /api/defects` 쿼리 파라미터·응답 모두 코드값(`CRACK`, `ACTION_PENDING` 등)을 사용한다.
...
### 4.2 `/ai/nl-search` 출력 → `GET /api/defects` 파라미터 매핑

`GET /api/defects`는 아직 미구현이라 이 매핑이 곧 그 쿼리 파라미터 설계의 기준이 된다.

| `/ai/nl-search` 응답 필드 | `/api/defects` 쿼리 파라미터(안) | 비고 |
|---|---|---|
...
1. 사용자가 자연어 질의 입력 → `POST /ai/nl-search` 호출.
2. 응답 성공 + `clarifying_question`이 `null`이면, 응답의 `filters`로 **기존 수동 필터 상태를 통째로 교체**한다(부분 병합 아님 — 자연어 질의 결과가 새 필터 기준이 된다).
3. `clarifying_question`이 있으면 필터 상태는 그대로 두고 질문만 노출한다(§2.3).
4. `unsupported_terms`가 있으면 인식된 나머지 필터는 적용하되, 무시된 표현을 안내한다(§2.3).
5. 필터 상태가 갱신되면 프론트는 동일한 `GET /api/defects` 조회 경로를 그대로 사용한다 — 수동 필터로 조회하든 자연어로 조회하든 최종 진입점은 하나다.

### 4.4 AI 실패 시 fallback 원칙

`/ai/nl-search`가 실패(`LLM_TIMEOUT`/`LLM_RATE_LIMIT`/`LLM_INVALID_OUTPUT`)하면:
...
- HAJA-120 후속 구현 티켓 범위: ① AI 서버 `/ai/nl-search` 구현, ② 백엔드 `GET /api/defects` 구현(§4.2 파라미터 기준), ③ 프론트 `inspection/types.ts` 영문 코드 마이그레이션 + 자연어 검색 UI(§4.3 동기화 로직).
...
- **AI 서버**: 5.1~5.4를 `test_nl_search_chain.py`의 케이스로 사용. 5.1(정상)·5.3(미지원 조건 포함)·5.4(빈 질의)는 `filters`/`unsupported_terms`가 표에 명시된 값과 **정확히 일치**하는지 검증한다. 5.2(애매한 질의)는 `clarifying_question` 문구까지 고정하지 않고 `filters == 표의 빈 값` && `clarifying_question is not None`라는 **구조적 조건**만 검증한다(§5.2 참고).
- **백엔드**: 5.1의 기대 `filters`를 §4.2 매핑표로 변환한 쿼리 파라미터가 `GET /api/defects`에서 올바른 결과를 반환하는지 검증.
- **프론트**: 5.1~5.4를 MSW mock 핸들러 fixture로 사용 — `clarifying_question`/`unsupported_terms` 분기 UI가 각각 올바르게 렌더링되는지 검증.

### 5.6 완료 기준 반영

- 정상/애매함/지원불가/빈 질의 케이스 전부 포함.
