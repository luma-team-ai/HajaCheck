...
**소유권 범위는 아래 클라이언트 필터보다 먼저 서버에서 강제한다.** 시설과 시설 경유 점검·하자 범위는 `facilities.company_id = authCompanyId`인 **단일 회사 스코프**다. Spring Boot는 매 요청 현재 DB에서 요청 사용자가 ACTIVE이고 `users.company_id = authCompanyId`인지, 회사가 `status=APPROVED` 및 `verification_status=VERIFIED`인지, `company_memberships` 행이 `status=APPROVED`, `approved_at IS NOT NULL`, `revoked_at IS NULL`, `expires_at IS NULL OR expires_at > now()`인지 모두 검증한다. 하나라도 실패하면 `403 FORBIDDEN`이며 principal에 남은 stale companyId를 신뢰하지 않는다.

관리자도 전체 시설물 조회 예외가 없고, `inspections.assigned_inspector_id = authUserId`라는 이유만으로 타회사 시설의 점검·하자를 포함하지 않는다. 담당자 배정 자체도 동일 회사에서만 허용한다. 모든 역할에서 `defects.is_deleted = false`를 추가로 강제한다. 클라이언트가 `facilityId`·`inspectionId`·회사 식별자 등으로 범위를 확장하거나 논리 삭제 행을 다시 포함시킬 수 없으며, 범위 밖 `defects`는 다른 필터 조건과 일치하더라도 결과에 포함하지 않는다.
...
**403과 빈 결과를 구분한다.** 미인증은 `401`, 인증됐지만 `GET /api/defects` 허용 역할이 아니면 `403`이다. 반면 허용 역할의 사용자가 소유한 시설·배정된 점검이 없거나, 서버 소유권 범위 안에서 필터와 일치하는 하자가 없거나, 일치 행이 모두 `is_deleted=true`라 제외된 경우는 정상 조회이므로 `200`과 `{ "content": [], "page": 요청 page, "size": 요청 size, "totalElements": 0, "totalPages": 0 }`을 반환한다. 타 사용자 데이터가 존재하더라도 현재 주체의 서버 유도 범위가 비어 있으면 그 존재를 드러내는 `403` 대신 같은 빈 페이지를 반환한다.
