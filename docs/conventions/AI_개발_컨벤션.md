...
**동기/비동기 경계**(v0.2 명확화, PR #240 4차 검수 P1 후속 — HAJA-208): 이 원칙은 **클라이언트 대면 경계**에 적용된다 — 장시간 작업(보고서 생성 등)을 사용자가 직접 호출하는 API(예: 백엔드 `/api/reports`)는 동기 응답 금지, 비동기 잡 패턴(잡 ID → 폴링) 사용. 반면 `verify_internal_key`로 보호되는 **내부 전용 `/ai/*` 엔드포인트**는 이 경계 안쪽(백엔드가 잡 워커에서 호출)이므로 동기 응답이 정상이다 — `/ai/report`가 4개 섹션을 동기로 생성해 반환하는 것은 설계 위반이 아니라 의도된 구조.
