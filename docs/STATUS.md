...
- **P1**
  ...
  - [ ] **dev→main 승격 대기** — #108·#111 등 + **기업 인증 3화면(#195·#196·#200·#191)** dev 반영분, 사람 승인(hold_promote). DB 전제 ✅확인(2026-07-15, companies/user_consents/enum 존재)
    - ✅ **승격 전 .env 주입 — 전부 완료**(2026-07-17 오후 실측): `AI_INTERNAL_KEY` · **SMTP 5키**(`SMTP_HOST`/`SMTP_USERNAME`/`SMTP_PASSWORD`/`MAIL_FROM`/`FRONTEND_BASE_URL`) · **`VITE_KAKAO_MAP_APP_KEY`**(#323/PR #344로 `:?` 가드 신설 → 같은 날 주입). **`.env`는 더 이상 차단 요소가 아니다** — 차단은 아래 인프라 2건뿐.
    - ⚠️ **이번 승격 = B전환 DB 컷오버 동반 릴리스**(2026-07-17 실측). dev가 main보다 **118커밋**(46 PR) 앞서며, 그 안에 #258/#259(bridge net + 전용 postgres/redis 컨테이너화)가 들어 있다. **지금 그대로 승격하면 프로덕션이 뜨지 않는다** — 실측 근거 3건:
      1. **redis 포트 충돌**: dev `arm1.yml`의 새 redis가 `127.0.0.1:6380`을 요구하는데, 기존 `hajacheck-redis` 컨테이너가 이미 점유 중(12일째) → `port already allocated`로 compose 기동 실패.
      2. **spring이 빈 DB를 봄**: main은 `127.0.0.1:5432/hajacheck`(호스트 PG, 라이브 데이터) ↔ dev는 `postgres:5432/...`(새 컨테이너, `pgdata` 볼륨 비어 있음). `ddl-auto=validate`라 테이블 부재 시 기동 실패.
      3. `FRONTEND_BASE_URL` 미주입(↓알려진 이슈).
    - **인프라만 체리픽·시간순 분할은 불가** — #236·#245가 같은 `arm1.yml`을 앱 동작 변경과 섞어 수정해 충돌한다. → **앱+인프라를 함께 올리는 단일 릴리스 + 컷오버 런북 실행**이 유일한 경로. 절차: `docs/_local/배포_B전환_구현설계_런북_2026-07-16.md`. 리허설·롤백 경로 확보 후 팀 합의하에 진행.
