...
- **기업 인증 플로우 3화면 풀스택 (→ dev, 2026-07-15)** — Figma 인증 화면 구현. 4개 PR 일괄 머지:
  ...
  - ⚠️ **승격 전 필수**: dev PG에 `companies`/`user_consents`/enum 3종(DDL v0.3) 반영돼야 배포 부팅 성공(ddl-auto=validate). 현재 DDL은 설계 참조용으로 배포 미실행 상태 → [알려진 이슈] 참조
...
- **P1**
  ...
  - [ ] **dev→main 승격 대기** — #108·#111 등 + **기업 인증 3화면(#195·#196·#200·#191)** dev 반영분, 사람 승인(hold_promote). ⚠️ **승격 전 dev PG에 companies/user_consents/enum DDL 반영 필수**(아래 알려진 이슈)
  - [ ] **후속 #194/HAJA-172** 비밀번호 찾기·새 비번설정 (보안질문 방식, SMTP 미사용) — 계정탈취 P1로 이번 범위 제외분
...
| **기업 인증 배포 전제: companies/user_consents/enum DDL 미반영** — DDL v0.3(`docs/design/db/HajaCheck_script_v0.3.sql`)의 companies·user_consents·3 enum(company_status_type·business_verification_status_type·consent_policy_type)이 설계 참조용으로 **dev PG 미적용**. 이 상태로 dev→main 승격→배포 시 Spring `ddl-auto=validate`가 테이블 부재로 **부팅 실패** | ⛔ **승격 차단** — 승격 전 운영자가 dev PG(oci-arm1 localhost:5432 hajacheck)에 해당 DDL 적용 필요. 적용 후 승격 |
