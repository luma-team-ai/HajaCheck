...
- **#179 `docs+chore: 로컬 개발 가이드 + 시드 테스트 유저 + MSW 토글`** (HAJA-165 → dev, 2026-07-15) — `@Profile("local")` 시드 유저 + 로컬 개발 가이드 + 프론트 MSW 토글. **PR머신 오프 → 메타 로컬 검수**(code-reviewer·security-reviewer opus, P1 0). 시드 ADMIN 유저를 `app.local-seed.enabled` 기본 **false(옵트인)**로 하드닝(공유 DB known-password 백도어 방지). 리뷰 P2(시드 경합 시 `@Transactional` 경계 때문에 catch 실효 없음→기동 실패) 픽스 후 재검토 PASS. CI green, squash `7a71053`. ⚠️ **main 미승격**(dev-pr-check)
