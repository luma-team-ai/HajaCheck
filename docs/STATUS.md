...
- **auth 화면 다듬기 (정재봉, 2026-07-19~20 진행 중 — 세션 이어받기용)**
  - ✅ 머지됨: #415(탭 50/50·이메일 placeholder·비번 강도미터·버튼 문구) · #422(로그인 탭 위치고정·배너/개인가입/협업자 삭제·로고 홈링크) · #427(가입 폼 테두리·이메일/비번 실시간 검증) · #428(가입 로고 홈링크)
  - 🕒 검수 대기: **#430 / HAJA-276**(로그인·가입 로고 통합 교체 + 개인탭 "아이디/비번찾기" 제거) — 머신 검수/머지 대기. **머지 후 워크트리 `../hajacheck-brand-logo` + 브랜치 `frontend/brand-logo` 정리** 필요.
  - 📌 **내일 팀협의**: 가입 화면(어두운 배경) 로고 흰색 처리(`brightness-0 invert`) 방향 결정 — 흰색 유지 vs 로고 뒤 흰 칩 vs 배경 밝게.
  - **후속 작업(합의)**:
    - **#417 / HAJA-269 이메일 도메인 selectbox** — email@[도메인 드롭박스: naver.com/gmail.com/직접입력] 분리 입력. **팀 요청 있음 → 다음 착수 1순위 후보.** (로그인·가입 이메일칸 적용)
