...
| FR-010 | 보고서 생성 요청 | `POST /api/reports` — 사용자는 검수 확정된 하자 목록을 바탕으로 점검 보고서 생성을 요청할 수 있어야 한다. | 점검자 | 상 |
| FR-011 | 보고서 PDF 다운로드 | `GET /api/reports/{id}/pdf` — 사용자는 생성된 보고서를 PDF 파일로 다운로드할 수 있어야 한다. | 점검자 | 중 |
| FR-012 | 하자 목록 검색·필터 | `GET /api/defects` — 사용자는 유형·등급·상태별로 하자 목록을 필터링·검색할 수 있어야 한다. | 점검자/관리자 | 중 |
| FR-013 | 하자 자연어 검색 | `POST /ai/nl-search` — 사용자는 자연어 질의를 입력하면 시스템이 이를 필터 조건으로 변환해 결과를 반환해야 한다. | 점검자 | 하 (P1) |
