...
| FR-013 | 하자·점검 자연어 검색 (P1) | 공개 `POST /api/defects/nl-search` — Spring Boot가 인증·`INSPECTOR` 또는 `ADMIN` 권한·`has_ai_addon`을 검사한 뒤 KST 기준일과 함께 내부 FastAPI `POST /ai/nl-search`로 전달하고, 자연어 질의를 하자 유형·등급·상태와 점검일·종류·진행상태·회차·전체 하자건수 필터로 변환해 반환해야 한다. 최종 필터는 `GET /api/inspections`의 반복 배열·포함 범위 파라미터로 적용한다. 회사 플랜 상속은 회사 `APPROVED`+`VERIFIED` 및 `company_memberships`의 유효한 승인 멤버십(승인·미회수·미만료·사용자 소속 일치)을 모두 검증해야 한다. | 점검자·회사 관리자(Standard 이상) | 하 |
...
## 변경 이력
- **v0.5 (2026-08-04)**: 상단 안내 정정 — 부분 문서임을 명시하고, openapi 실측(v0.42.0-draft · 81 path · 95 operation)·contract v0.15를 병기. "FR 번호 확정되면 일괄 치환 예정"이 1개월 넘게 방치돼 있어 **PRD FR 체계가 기준**임을 명시(2026-08-04 공개문서 stale 감사).
