...
2. **단위 시트 상단 전체목록 ↔ 메뉴별 시트 결과 상충**
   - UT-031·032·034: 상단 **PASS**(07-30) ↔ 메뉴별 **미수행**
   - UT-082: 상단 **미수행** ↔ 메뉴별 **PASS**(08-04)
...
> §2~§6에서 확인된 불일치를 **셀 단위 작업 지시**로 정리한 것. 시트 담당자가 이 표대로 반영한다.
> 각 행의 "근거"는 코드 실측이며, 판단이 필요한 항목은 별도 표기했다.
> **범위 결정 반영(2026-08-04)**: **영상 업로드는 범위 밖으로 확정**한다 — 아래 1번.

### 8-1. 통합 시나리오 시트 (8건)

| # | 위치 | 현재 | 정정 | 근거 |
|---|---|---|---|---|
| 1 | **HC_007_02** 테스트데이터 | 이미지 다수, **영상 1건** | 이미지 다수 | `allowedContentTypes` = `image/jpeg`·`image/png` |
| | 〃 예상결과 | 진행률 표시, **영상은 프레임 자동 추출** | 업로드 진행률 표시 | 영상은 `FILE_INVALID_TYPE`로 거부 |
| | 〃 수행결과 | PASS | **미수행** + 비고 "영상 업로드는 범위 밖(이미지 전용) — UT-082와 정합" | **범위 결정 확정** |
| 2 | **HC_015_05** 예상결과 | …서버 자원, **예산 가드레일 경고 상태**가 대시보드에 표시 | …서버 자원까지만(가드레일 문구 삭제) → **PASS 유지 가능** | `SystemMonitoringResponse` 4종 |
| 3 | **HC_014** 검증포인트 5 | …에러로그,자원,**HF사용량** | …에러로그,자원 | HF 카드는 서버 자원 카드로 대체(#728) |
| 4 | **HC_003** 시나리오행 + `_01`·`_02`·`_04` | 이메일 **인증코드** 발송·검증 | 아이디 찾기 = **사업자번호 + 상호명/대표자명 대조** → 마스킹 이메일 / 비밀번호 = **재설정 링크 + 1회성 토큰** | `AccountRecoveryService`·`PasswordResetService`. **절차 기술만 정정, `_02` PASS는 유지** |
| 5 | **HC_001_03·04** 비고 | N/A 설계변경으로 **코드 없음** | **비고 삭제 → 수행 대상으로 환원** | 진위확인 버튼·뱃지 6종·가입 게이팅 전부 구현(#663) |
| 6 | **HC_016_07** 예상결과 | `/status`와 **분리되어 조치 필드만** 저장 | **`targetStatus`로 상태 전이 + 조치 4필드 저장**(총 5필드 필수) | #1128, `openapi.yaml` |
| 7 | **HC_005_08** 테스트데이터 | 120 | **121** | `@Max(120)` — 120은 통과가 정상 |
| 8 | **HC_005_02** 테스트데이터 | 유 미입력 | 위치 미입력 | 단순 오타 |

> **HC_001_02**(이메일 [중복확인] 버튼)는 **비고 유지**가 맞다 — `GET /api/auth/email-availability`는
> 있으나 가입 화면에 버튼 UI가 실제로 없다.

### 8-2. 단위 시트 (6건)

| # | 위치 | 정정 | 근거 |
|---|---|---|---|
| 1 | **UT-074** 예상결과 | "`defect.status`는 변경 없음" → **"`targetStatus`로 전이 + 조치 필드 갱신"** | §2-1 |
| 2 | **UT-073** 케이스내용 | 4필드 → **5필드**(`targetStatus` 포함) | §2-1 |
| 3 | **UT-081** 케이스내용 | 등록 폼은 **`@Min(0)` 허용**("주기 미설정") 명시 — 0 차단은 점검주기 설정 요청만 | §5-3 |
| 4 | **UT-085** | 중복 등재 1건 제거 | §6-5 |
| 5 | 결과 통일 | **UT-031·032·034 → 미수행**(자동 테스트 부재) · **UT-082 → 미수행**(설정만 존재, 검증 테스트 없음) | §5-1 · §6-2 |
| 6 | 미수행 → **PASS 처리 가능** | **UT-022~025 · 066~070 · 079 · 080** (11건) | §5-2 — 대응 자동 테스트 실재 |

> **순증감 정리**: 현재 미수행 14건에서 6번으로 11건이 빠져 **3건**(UT-078·081·082)이 되고,
> 5번으로 UT-031·032·034가 미수행으로 들어와 **최종 6건**이 된다. 겉보기 수치는 줄지만
> "근거 없는 PASS"가 사라지므로 **시트 신뢰도는 올라간다** — 6번만 반영하고 5번을 빼면 안 된다.

### 8-3. 시트로는 해결되지 않는 것

| 항목 | 이유 |
|---|---|
| **§4-5 AI 주간 브리핑 메뉴** | 시트 비고("메뉴 삭제되었음")는 **이미 정확**하다. 틀어진 쪽은 코드 5곳(시드·폴백·목·테스트·PRD) |
| **HC_016_06 dead code** | 시트 FAIL 기록도 정확. 프론트 배선 또는 컴포넌트 제거 결정이 필요 |
| **§6-1 케이스 ID 오프셋** | 단순 치환이 아니라 ID 체계 재정렬이라 별도 합의 필요 |

### 8-4. PRD 영향 없음 확인

**영상 범위 밖 확정은 PRD 수정을 유발하지 않는다** — PRD는 이미 `:529`·`:702`에서 "영상(MP4) 프레임
자동 추출은 현재 미구현, 이번 범위는 이미지 전용"으로 정확히 기술하고 있다. 이 건은 **시트만 stale**했다.
