...
## 코드 리뷰 기준 (전역 보완 — 이 레포 리뷰 관점)
> 전역(시크릿 스캔·PR/워크트리·P1/P2/P3 사이클 매트릭스)과 스택 컨벤션(Java/FastAPI/React)에 이미 있는 룰은 반복하지 않는다. 아래는 이 레포에서 리뷰 시 **특히 볼 관점**만 — 사람·PR머신·Claude 공통. 발견 항목엔 **P등급 + 파일:라인 + 근거 + 수정안**을 함께 단다.

### 인증·인가·개인정보 (대개 P1)
- **인가(소유자·회사 스코프) 검증**: 리소스 조회·수정 전 요청자와 소유자/회사 소속 일치를 확인해 **IDOR·cross-tenant를 차단**한다. "인증됨 ≠ 인가됨". (점검 회사 스코프 정책과 정합 — 개인=보고서 수신 전용, 점검 생성·담당자 배정=회사 소속 한정)
...
- **경계·정합 계산**: 배열·페이지 경계, 날짜·금액·권한 등 경계 계산은 근거를 갖고 검증한다(Trivial↑ 판단 기준과 정합).
