...
# ---------------------------------------------------------------------------
# 실제 Chroma 기반 — enforce_retention / purge_semantic_cache
# ---------------------------------------------------------------------------
...
def test_enforce_retention_evicts_oldest_entries_over_capacity(
    ...
    with patch.object(semantic_cache, "now_epoch_seconds", return_value=1_000_000):
        _add_entry(real_cache_collection, "oldest", created_at=999_910)
        _add_entry(real_cache_collection, "middle", created_at=999_920)
        _add_entry(real_cache_collection, "newest", created_at=999_930)
        enforce_retention()

    assert sorted(real_cache_collection.get(include=[])["ids"]) == ["middle", "newest"]
...
def test_purge_semantic_cache_clears_all_entries(real_cache_collection):
    _add_entry(real_cache_collection, "a", created_at=1)
    _add_entry(real_cache_collection, "b", created_at=2)

    assert purge_semantic_cache(COLLECTION_REGULATIONS) == 2
    assert real_cache_collection.count() == 0


def test_purge_semantic_cache_on_empty_collection_is_safe(real_cache_collection):
    """chromadb는 빈 id 리스트 삭제를 ValueError로 거절한다(실측) — 빈 캐시 purge가 터지면 안 된다."""
    assert purge_semantic_cache(COLLECTION_REGULATIONS) == 0
    assert real_cache_collection.count() == 0


def test_purge_semantic_cache_is_noop_for_non_regulations(real_cache_collection):
    """defect_kb는 `hybrid_search` 대상이 아니라 RAG 답변에 영향이 없다 — bm25_index.invalidate와
    동일한 no-op 규약(무관한 변경으로 캐시를 통째로 날리지 않는다)."""
    _add_entry(real_cache_collection, "a", created_at=1)

    assert purge_semantic_cache(COLLECTION_DEFECT_KB) == 0
...
def test_delete_document_purges_semantic_cache():
    ...
    with patch.object(rag_ingest, "get_vectorstore"), patch.object(
        rag_ingest, "purge_semantic_cache"
...
def test_ingest_into_defect_kb_does_not_purge_regulations_cache():
    ...
    # purge 함수를 mock하지 않고 **실제로 태운다** — no-op 규약이 깨지면 캐시 컬렉션에 접근하게
