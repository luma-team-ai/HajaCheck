...
EasyOCR→RapidOCR PP-OCRv5 한국어 교체 #722).
...
- 파싱 유닛(실제 OCR 아님): RapidOCR 호출 결과(`res.txts`/`res.scores` 튜플)를 모킹해
  `_extract_text_lines`가 실제 이미지 픽스처 경로로도 정상 파싱하는지 확인. 모델 다운로드가
  필요한 실제 추론은 AI_개발_컨벤션 "HF 모델 관련 테스트는 항상 모킹" 원칙에 따라 모킹으로
  대체한다. RapidOCR 엔진 자체의 wiring(`Global.use_cls=False` 등)은 `tests/test_ocr_client.py`
  에서 별도 검증.
"""
...
def test_run_chain_prefers_regex_reg_number_over_llm_guess(mock_get_engine, mock_get_llm):
    ...
    mock_engine.return_value = SimpleNamespace(
        txts=("등록번호:123-45-67890", "상호:하자체크 주식회사", "대표자: 홍길동"),
        scores=(0.97, 0.99, 0.86),
    )
    mock_get_engine.return_value = mock_engine
...
def test_run_chain_falls_back_to_llm_number_when_regex_finds_none(
    ...
    mock_engine = MagicMock()
    mock_engine.return_value = SimpleNamespace(txts=("등록번호 확인불가",), scores=(0.4,))
    mock_get_engine.return_value = mock_engine
...
def test_run_chain_normalizes_business_start_date_from_llm(mock_get_engine, mock_get_llm):
    ...
    mock_engine = MagicMock()
    mock_engine.return_value = SimpleNamespace(txts=("개업연월일:2020.01.15",), scores=(0.9,))
    mock_get_engine.return_value = mock_engine
...
def test_run_chain_business_start_date_null_when_llm_gives_nothing(mock_get_engine, mock_get_llm):
    ...
    mock_engine = MagicMock()
    mock_engine.return_value = SimpleNamespace(txts=("사업자등록증",), scores=(0.9,))
    mock_get_engine.return_value = mock_engine
...
# ── 파싱 유닛: RapidOCR 응답 형식 모킹 + 실제 이미지 픽스처 경로(디코딩 파싱 확인, 실제 OCR 아님) ──


@patch("ai.chains.business_license_ocr_chain.get_ocr_engine")
def test_extract_text_lines_parses_rapidocr_format_with_real_fixture_path(mock_get_engine):
    """`_extract_text_lines`가 실제 이미지 픽스처 파일 경로로 RapidOCR 호출 결과
    (`res.txts: tuple[str, ...]`, `res.scores: tuple[float, ...]`)를 올바르게 파싱하는지
    확인한다.

    코드 리뷰 P3 — 이 테스트는 **실제 OCR 파이프라인을 검증하지 않는다**(`get_ocr_engine`을
    완전히 모킹하므로 실제 RapidOCR 모델은 전혀 호출되지 않는다). 픽스처 이미지는
    `_extract_text_lines`가 bytes를 읽어 반환값을 파싱하는 경로만 실경로로 태우기 위해
    재사용할 뿐이다(즉 "스모크 테스트"가 아니라 파싱 로직 유닛 테스트). 실제 RapidOCR
    PP-OCRv5 한국어 인식 정확도(완전일치 18/20)는 이 작업(#722) 배경 조사의 로컬 실측(모델
    다운로드 포함, 이 테스트 스위트 밖에서 별도 확인)으로 검증됨 — CI에서 매번 실제
    파이프라인을 돌리려면 네트워크 모델 다운로드가 필요하므로(AI_개발_컨벤션 "HF 모델 관련
    테스트는 항상 모킹" 원칙과 상충) 여기서는 모킹으로 대체한다.
    """
    mock_engine = MagicMock()
    mock_engine.return_value = SimpleNamespace(
        txts=("등록번호:123-45-67890",), scores=(0.97,)
    )
    mock_get_engine.return_value = mock_engine
...
# ── tracing_context 제외 검증 (#1534) ──────────────────────────────────────


@patch("ai.chains.business_license_ocr_chain.tracing_context")
@patch("ai.chains.business_license_ocr_chain.get_ocr_engine")
@patch("ai.chains.business_license_ocr_chain.get_llm")
def test_ocr_chain_disables_tracing_for_llm_call(
    mock_get_llm, mock_get_ocr_engine, mock_tracing_context
):
    """OCR 체인의 LLM 호출이 tracing_context(enabled=False) 블록 내에서 실행됨을 검증.

    PR머신 P1 반려(개인정보 국외 제3자 제공) 대응 — OCR 원문(사업자등록증)이
    LangSmith로 전송되지 않도록 트레이싱 자체를 차단한다.
    """
    # Mock OCR engine: 정상 인식 결과
    mock_engine = MagicMock()
    mock_engine.return_value = SimpleNamespace(
        txts=("사업자등록번호: 123-45-67890", "상호: 테스트회사"), scores=(0.95, 0.92)
    )
    mock_get_ocr_engine.return_value = mock_engine

    # Mock LLM: structured output 정상 반환
    mock_llm = MagicMock()
    mock_llm.with_structured_output.return_value.invoke.return_value = (
        BusinessLicenseOcrExtract(
            business_registration_number="123-45-67890",
    ...
    # Mock tracing_context: context manager로 작동, enabled 파라미터 검증
    mock_tracing_context_instance = MagicMock()
    mock_tracing_context.return_value = mock_tracing_context_instance
    mock_tracing_context_instance.__enter__ = MagicMock(return_value=mock_tracing_context_instance)
    mock_tracing_context_instance.__exit__ = MagicMock(return_value=None)

    # 체인 실행
    result = run_business_license_ocr_chain("iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR42mNk+M9QDwADhgGAWjR9awAAAABJRU5ErkJggg==")

    # 검증: tracing_context(enabled=False)가 호출됐는지 확인
    mock_tracing_context.assert_called_once_with(enabled=False)
    mock_tracing_context_instance.__enter__.assert_called_once()
    mock_tracing_context_instance.__exit__.assert_called_once()

    # 체인이 정상 완료되었는지 확인
    assert result.business_registration_number == "123-45-67890"
    assert result.company_name == "테스트회사"
    assert result.representative_name == "김테스트"
