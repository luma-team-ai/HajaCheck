...
LangSmith 트레이싱(#1534): 트레이싱을 켜면 프롬프트(=OCR 원문 전체)와 LLM 응답이 외부 LangSmith
서버로 그대로 전송된다. 기존 #552의 "로그에 개인정보 평문 금지" 원칙과 별개로, 트레이싱 경로에서는
마스킹이 완전히 제거됐다(2026-07-29 정재봉 재승인, #1534). 관측 편의를 위해 개인정보를 제3자 SaaS에
넘기는 것으로 정책이 변경됐다.
"""
import base64
import binascii
