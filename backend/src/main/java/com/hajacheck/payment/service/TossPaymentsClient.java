 ...
 *
 * <p><b>재시도하지 않는다</b>: 승인은 멱등이 아닌 <b>돈이 움직이는 쓰기</b>다. 타임아웃 후 재시도하면 PG
 * 쪽에서 이미 승인된 건을 다시 건드려 중복 청구·상태 불명을 키울 수 있다. 타임아웃은 실패로 확정하고,
 * 사용자가 같은 orderId 로 다시 확정을 시도하는 경로(멱등 처리)로 흡수한다.
 *
...
public class TossPaymentsClient {
    ...
    public TossPaymentApproval confirm(String paymentKey, String orderId, long amount) {
        ...
        try {
            ...
            throw new TossPaymentApprovalException(
            ...
            throw new TossPaymentApprovalException(error.code(), error.message());
