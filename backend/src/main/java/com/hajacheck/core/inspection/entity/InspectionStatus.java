...
public enum InspectionStatus {
    ...
    //  - ANALYZING → ANALYZED(분석 성공, 실패 0장) / FAILED(사진 하나라도 분석 실패) /
    //    CREATED·UPLOADING·ANALYZED(큐포화 롤백·고착 복구·리퍼 복원 — 모두 직전 상태
    //    statusBeforeAnalysis 로 되돌림, RECOVERY_STATUS=UPLOADING 포함)
    //  - ANALYZED → ANALYZING(재분석) / REVIEWED(검수 확정, 현재 전이 코드는 미구현이나 들어올 때
    //    이 테이블을 통해 안전 배선되도록 미리 허용)
    //  - FAILED → ANALYZING(사진 삭제 또는 재분석 버튼으로 재시도 — 트리거 코드는 별도 배선 필요,
    //    이 테이블엔 안전 배선 목적으로 미리 허용)
    ...
    static {
        ...
        transitions.put(ANALYZING, EnumSet.of(CREATED, UPLOADING, ANALYZED, FAILED));
        transitions.put(ANALYZED, EnumSet.of(ANALYZING, REVIEWED));
        transitions.put(FAILED, EnumSet.of(ANALYZING));
