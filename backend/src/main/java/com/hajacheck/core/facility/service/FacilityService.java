...
public class FacilityService {
    ...
    // 지도 전용 대량 반환 관측(#1656 리뷰 P2) — listForMap()은 FACILITY_LIST_MAX 같은 하드 캡을 의도적으로
    // 두지 않는다(무상한이 이 엔드포인트의 취지). 다만 회사 시설물이 비정상적으로 많아지면(데이터 이상·잘못된
    // 시드 등) 운영 감지가 필요하므로, list()의 상한 도달 WARN 관례와 동일하게 임계값 도달 시 WARN만 남기고
    // 응답은 그대로 전체 반환한다(차단 아님).
    private static final int FACILITY_MAP_LARGE_RESULT_WARN_THRESHOLD = 5000;
    ...
    public FacilityMapResponse listForMap(Long userId, Long companyId) {
        ...
        // 하드 캡은 두지 않는다(무상한 취지 유지) — 임계값 도달은 차단이 아니라 운영 관측용 WARN 로그다.
        if (facilities.size() >= FACILITY_MAP_LARGE_RESULT_WARN_THRESHOLD) {
            log.warn("지도 시설물 목록 대량 반환 — companyId={} {}건 (관측 임계값 {} 이상, 상한 없이 전체 반환)",
                    companyId, facilities.size(), FACILITY_MAP_LARGE_RESULT_WARN_THRESHOLD);
        }
        if (facilities.isEmpty()) {
