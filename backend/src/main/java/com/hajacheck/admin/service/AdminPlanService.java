 ...
 * 회사 플랜을 상속하지 않으므로(§2.6 "미승인 멤버십은 상속 대상 아님") PLAN_NOT_FOUND 로 응답한다(#363,
 * {@link #resolveInheritedCompanyId} 참고 — 가입 시 오너를 PENDING 멤버십으로 생성하고 승인 시점에
 * APPROVED 로 전이시키는 CompanyApprovalService 가 이 계약의 전제조건이다).
 *
...
public class AdminPlanService {
    ...
    //
    // (#887 임시 완화 → #363 으로 정식 복원) 가입 시 멤버십 행을 안 만들어 모든 기업 관리자가 PLAN_NOT_FOUND
    // 를 맞던 결손은 #887 에서 이 검사를 통째로 빼는 것으로 임시 봉합했었다. #363 이 가입 경로
    // (CompanyAccountWriter)에서 오너를 PENDING 멤버십으로 생성하고, 승인 엔드포인트(CompanyApprovalService)
    // 가 실제로 배선되면서 그 결손 자체가 해소됐다 — 임시 완화를 걷어내고 원래 계약(유효 승인 멤버십 필수)을
    // 복원한다. 승인 전 관리자가 이 화면에서 PLAN_NOT_FOUND 를 보는 것은 이제 버그가 아니라 의도된 동작이다.
