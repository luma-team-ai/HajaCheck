...
--
-- 배경:
--   V35(baseline 스탬프 누락 복구, #1306)가 seed한 대시보드 자식 메뉴는
--   DASHBOARD_OVERVIEW(39) · DASHBOARD_UPCOMING_INSPECTIONS(40) 둘뿐이다.
--   그런데 menus 시드 id 30~59 구간에서 **41만 결번**이고, 위치가 정확히 40과
--   FACILITIES_LIST(42) 사이다 — 대시보드 3번째 자식이던 행이 복구 대상에서
--   빠진 것으로 판단한다. V36 주석의 실측치("menus 29행" = 30행 중 41 결번)와도 일치한다.
--
--   그 결과 실서버 사이드바에 "AI 주간 브리핑 카드" 링크가 뜨지 않았다
--   (통합테스트 HC_004_03 "해당 링크 메뉴 삭제되었음").
--
