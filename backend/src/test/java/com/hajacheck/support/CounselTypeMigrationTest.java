...
/**
 * V14(add_counsel_type, #743)이 기존 행이 있는 구스키마에 대해 어떻게 동작하는지 고정하는 문서화 테스트.
 *
 ...
 * 실측 0건임을 직접 확인(2026-07-24, V14__add_counsel_type.sql 헤더 주석 참고)해 현재 설계를 그대로
...
class CounselTypeMigrationTest {
    ...
    static final PostgreSQLContainer<?> POSTGRES = new PostgreSQLContainer<>("postgres:16")
            .withDatabaseName("hajacheck_v14")
            .withUsername("postgres");

    @Test
    void V14는_counsel_type없는기존행이있으면_NOT_NULL위반으로실패한다() {
        DataSource dataSource = new DriverManagerDataSource(
                POSTGRES.getJdbcUrl(), POSTGRES.getUsername(), POSTGRES.getPassword());
        JdbcTemplate jdbc = new JdbcTemplate(dataSource);

        // 1) V1~V13까지만 적용(counsel_type을 추가하는 V14 적용 전 상태 확보 — V13은 media.detail_url).
        migrateTo(dataSource, "13");

        // 2) counsel_type 컬럼이 아직 없는 구스키마에 counsel_tickets 행 1건을 심는다.
        Long userId = jdbc.queryForObject("""
                insert into users (email, name, role, password_hash)
                values ('v14-counsel-legacy@haja.test', 'V14 레거시 상담요청자', 'USER'::role_type,
                        'test-password-hash')
                returning id
                """, Long.class);
        jdbc.update("""
        ...
        // 3) V14 forward-apply — 현재 설계(DEFAULT 없는 NOT NULL)이므로 반드시 실패해야 한다.
        //    실패하지 않고 통과해 버리면, 이 계약(prod 0건 근거로 DEFAULT를 안 붙이기로 한 결정)이
        //    조용히 깨진 것이므로 이 테스트가 대신 실패해 알려준다.
        assertThatThrownBy(() -> migrateTo(dataSource, "14"))
