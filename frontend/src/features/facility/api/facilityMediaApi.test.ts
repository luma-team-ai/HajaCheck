...
// 것이라 이 realm 불일치 자체가 없다 — 이 파일이 "업로드 경로에서 multipart가 실제로
// 올바르게 조립되는지"를 바이트 단위로 검증하는 자리이므로, realm을 우회하는 잔재주 대신
// 애초에 문제가 없는 환경을 쓰는 쪽을 택했다. 대신 jsdom이 "공짜로" 제공해 주던 것들을 이
// 테스트에서만 최소한으로 메꿔준다(다른 테스트 파일에는 영향 없음 — vitest는 테스트 파일마다
// 모듈 레지스트리를 격리한다):
