...
// logout() 실패로 서버 세션이 남아있을 수 있는 경우 — PlatformAdminLoginPage와 동일 이유로
// 일반 role 불일치 메시지와 구분해 안내한다(PR머신 리뷰 P3, #558).
const LOGOUT_FAILED_MESSAGE = '상담원 계정이 아닙니다. 세션 정리에 실패했으니 브라우저를 종료한 뒤 다시 시도해 주세요.';
...
// 폼이며, authApi.login()은 재사용하되 role 검사·세션 무효화는 useCounselorLogin이 담당한다.
export function CounselorLoginPage() {
  const [loginId, setLoginId] = useState('');
  const [password, setPassword] = useState('');
  const navigate = useNavigate();
  const currentUser = useAuthStore((state) => state.user);
  const { login, isPending, error, roleDenied, logoutFailed } = useCounselorLogin();
