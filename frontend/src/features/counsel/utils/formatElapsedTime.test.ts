...
describe('formatElapsedTime', () => {
  ...
  it('1분 미만이면 방금 전을 반환한다', () => {
    const occurredAt = new Date('2026-07-27T11:59:30.000Z').toISOString();
    expect(formatElapsedTime(occurredAt, now)).toBe('방금 전');
  });

  it('1시간 미만이면 분 단위로 반환한다', () => {
    const occurredAt = new Date('2026-07-27T11:45:00.000Z').toISOString();
    expect(formatElapsedTime(occurredAt, now)).toBe('15분 전');
  });

  it('24시간 미만이면 시간 단위로 반환한다', () => {
    const occurredAt = new Date('2026-07-27T10:00:00.000Z').toISOString();
    expect(formatElapsedTime(occurredAt, now)).toBe('2시간 전');
  });

  it('24시간 이상이면 일 단위로 반환한다', () => {
    const occurredAt = new Date('2026-07-25T12:00:00.000Z').toISOString();
