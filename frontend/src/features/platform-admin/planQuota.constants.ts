...
// 플랫폼 관리자 > 플랜·쿼터 관리 라벨·임계값·플랜 상세 — features/admin/planQuota.constants.ts(#508)를
// 그대로 옮긴 것(#625). 플랜 이름 라벨은 사용자 관리와 동일하므로 constants.ts의 PLAN_LABEL을 재사용한다.
...
// 여기 하드코딩하지 않고 plans 테이블(GET /api/platform-admin/plans → AdminPlanCatalogItem)에서 가져온다.
