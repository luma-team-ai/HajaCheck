...
describe('mockPlatformAdminUsers lastAccessAt', () => {
  beforeEach(() => {
    vi.resetModules();
  });
  ...
  it('실시간이 흘러도 접근 시점의 Date.now() 기준으로 재계산된다', async () => {
    const { mockPlatformAdminUsers } = await import('./platformAdminUsers.mock');
    const HOUR = 60 * 60 * 1000;

    const firstReading = new Date(mockPlatformAdminUsers[0].lastAccessAt as string).getTime();
    ...
    const secondReading = new Date(mockPlatformAdminUsers[0].lastAccessAt as string).getTime();
  ...
  it('미접속 사용자(lastAccessOffsetMs=null)는 계속 null이다', async () => {
    const { mockPlatformAdminUsers } = await import('./platformAdminUsers.mock');
    const neverLoggedIn = mockPlatformAdminUsers.find((user) => user.id === 14);
