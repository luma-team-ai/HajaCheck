...
// 마이페이지 — 내 점검 이력 / 보고서 (HAJA-366/#668 / Figma 시안, BE 연동 #844/HAJA-442).
// MyPlanPage(#212)/MyProfilePage(#659)와 동일하게 흰 카드 하나 안에 섹션을 세로로 구성한다. 다만 이
// 화면은 기존 divide-y 셸 대신 탭으로 콘텐츠 영역을 전환한다 — 헤더(제목·부제·기간 필터)와 KPI 4종은
// 두 탭 모두에 공통으로 걸리는 요약 정보라 항상 노출하고, 탭에 따라 아래 콘텐츠만 바뀐다:
//  - '점검 이력' 탭: 점검 이력 테이블(+페이지네이션)
//  - '내 보고서' 탭: '최근 발급된 보고서' 카드 목록(회색 박스)
// 기간 필터(PeriodFilterSelect)는 실제 쿼리 파라미터로 승격됐다 — summary는 계약상 period가 없어
// 전체 기간 고정, 목록/보고서는 period를 그대로 넘긴다.
export function MyInspectionsPage() {
  ...
  const inspectionsQuery = useMyInspections({ page, size: pageSize, period });
  const reportsQuery = useMyReports(period);
  ...
  function handlePeriodChange(next: PeriodFilterValue) {
    setPeriod(next);
    setPage(1); // 기간이 바뀌면 이전 페이지 번호가 새 결과 범위를 벗어날 수 있어 1페이지로 되돌린다
  }
