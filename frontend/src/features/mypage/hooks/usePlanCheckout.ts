...
// 토스페이먼츠 결제창 연동(#989, HAJA-490) — 모의 결제(구 useCheckout, #712)를 대체한다.
// 1) SDK 로드(키 미설정 시 TossClientKeyMissingError로 즉시 실패, "조용한 undefined" 금지)
// 2) POST /me/plan/orders로 주문 생성 — amount는 서버 응답이 source of truth
// 3) requestPayment(successUrl/failUrl)로 결제창을 띄운다 — 정상 흐름이면 브라우저가 결제창으로
//    이동(redirect 방식)하므로 이 mutation의 Promise는 통상 resolve되지 않고 페이지가 전환된다.
//    사용자가 결제창을 취소하거나 SDK/주문 생성이 실패한 경우에만 reject되어 onError로 잡힌다
//    (UI 갱신 자체는 성공 시 /payments/success 라우트가 담당 — 여기서 쿼리 무효화하지 않는다).
export function usePlanCheckout() {
  return useMutation<void, unknown, PlanName>({
    mutationFn: async (planName) => {
      const tossPayments = await loadTossPaymentsSdk();
      const order = await mypageApi.createPlanOrder(planName).then((res) => res.data);
