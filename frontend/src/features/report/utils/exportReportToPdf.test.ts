...
describe('exportReportToPdf', () => {
  ...
  it('한글 폰트를 임베딩하고 content 섹션들을 렌더링한 뒤 Blob을 반환한다', async () => {
    ...
    expect(mockAddFont).toHaveBeenCalledWith('NotoSansKR-Regular.ttf', 'NotoSansKR', 'normal');
    expect(mockAddFont).toHaveBeenCalledWith('NotoSansKR-Bold.ttf', 'NotoSansKR', 'bold');
  ...
  it('실백엔드 상세 응답 fixture의 content로 PDF Blob을 생성한다', async () => {
    ...
    expect(findTableOptions((options) => Array.isArray(options.head))).toBeDefined();
    expect(mockOutput).toHaveBeenCalledWith('blob');
    expect(blob).toBeInstanceOf(Blob);
  });
  ...
  it('제출문·참여기술진 명단을 sectionOrder에 지정한 위치에 렌더링한다(수동 섹션)', async () => {
    const content = makeContent({
      manualSections: [
        ...
        },
        {
          ...
          type: 'participants',
          title: '참여기술진 명단',
          data: { entries: [{ role: '사업책임기술인', name: '김철수', qualification: '토목기사', period: '2026.01~06' }] },
        },
      ],
      sectionOrder: ['manual-submission-1', 'overview', 'summary', 'detail', 'recommendation', 'manual-participants-1'],
    });
    ...
    const participantsOptions = findTableOptions((options) =>
      Array.isArray(options.head) && (options.head as string[][])[0]?.includes('자격 및 주요경력'),
    );
    expect(participantsOptions?.body).toEqual([['사업책임기술인', '김철수', '토목기사', '2026.01~06']]);
  ...
  it('부위별 사진도 다른 섹션과 동등하게 sectionOrder로 자유롭게 재배치된다', async () => {
    ...
    const content = makeContent({
      sectionOrder: ['photos', 'overview', 'summary', 'detail', 'recommendation'],
    });
  ...
  it('사진이 0장이면 sectionOrder에 있어도 부위별 사진 섹션과 번호를 만들지 않는다', async () => {
    const content = makeContent({ sectionOrder: ['photos', 'overview', 'summary', 'detail', 'recommendation'] });

    await exportReportToPdf(content, { defectImages: [] });

    const renderedText = mockText.mock.calls.map(([text]) => text).flat();
    expect(renderedText.some((text) => typeof text === 'string' && text.includes('부위별 사진'))).toBe(false);
  ...
  it('삭제된 수동 섹션의 잔여 sectionOrder id는 무시한다', async () => {
    const content = makeContent({
      manualSections: [],
      sectionOrder: ['manual-submission-stale', 'overview', 'summary', 'detail', 'recommendation'],
    });
  ...
  it('흑백 인쇄 서식이므로 글자·괘선은 검정, 표 헤더 배경은 회색만 쓴다', async () => {
    ...
    const styles = options?.styles as { textColor: number[]; lineColor: number[] };
  ...
  it('머리말·꼬리말·페이지번호를 넣지 않는다(원본 서식에 없음)', async () => {
    ...
    const renderedText = mockText.mock.calls.map(([text]) => text).flat();
    expect(renderedText.some((text) => typeof text === 'string' && /^\d+\s*\/\s*\d+$/.test(text))).toBe(false);
  });
  ...
  it('부재별 상태평가 등급은 소문자로 적는다(대문자는 시설물 종합 안전등급 전용)', async () => {
    ...
    expect(options?.body).toEqual([['1', '1층 벽체', 'c', '균열', '설명', '원인']]);
  });
  ...
  it('등급별 건수에서 최악 등급을 상태평가 결과로 표기한다', async () => {
    ...
    const options = findTableOptions((candidate) =>
      ...
      && String((candidate.body as string[][])[0]?.[1] ?? '').startsWith('상태평가 결과 :'),
    );
  ...
  it('점검 축소본을 부위별 사진 표 안에 안전하게 배치한다(자동 페이지분할되는 autoTable 셀)', async () => {
    ...
    didDrawCell({ section: 'body', row: { index: 0 }, cell: { x: 23, y: 50, width: 164, height: 96 } });
    expect(mockAddImage).toHaveBeenCalledWith(expect.any(String), 'JPEG', 25, 52, 160, 92);

    mockAddImage.mockClear();
    // 캡션 행(홀수 인덱스)이나 head/foot 섹션에서는 이미지를 그리지 않는다.
    didDrawCell({ section: 'body', row: { index: 1 }, cell: { x: 23, y: 150, width: 164, height: 9 } });
  ...
  it('축소본이 없으면 빈 사진 대지를 만들지 않는다', async () => {
    ...
    const renderedText = mockText.mock.calls.map(([text]) => text).flat();
    expect(renderedText.some((text) => typeof text === 'string' && text.startsWith('부위별 사진'))).toBe(false);
    expect(mockAddImage).not.toHaveBeenCalled();
  });
  ...
  it('context에 없는 작성 기준일은 현재 날짜로 채우지 않고 빈 값으로 둔다', async () => {
    ...
    const row = (options?.body as string[][]).find((candidate) => candidate[0] === '작성 기준일');
    expect(row?.[1]).toBe('-');
  });
  ...
  it('미검증 법령 근거는 공식 근거처럼 표시하지 않고 미검증 표식을 붙인다', async () => {
    ...
    expect(options?.body).toEqual([['1', '1층 벽체', '보수', '중', '관련 근거 없음 (미검증)']]);
  });
