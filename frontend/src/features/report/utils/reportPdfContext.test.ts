...
/**
 * #1333 — PDF "부위별 사진"이 ①하자 좌표를 버려 박스가 안 그려지고 ②하자 1건당 사진 1장을 만들어
 * 같은 사진이 중복되던 회귀를 고정한다.
 */
function buildReport(defects: Array<Record<string, unknown>>): ReportDetailResponse {
  return {
...
const CRACK_A = {
  id: 1, mediaId: 101, type: 'CRACK', typeLabel: '균열', grade: 'C',
  bboxX: 0.32, bboxY: 0.1, bboxW: 0.05, bboxH: 0.71,
};
const CRACK_B = {
  id: 2, mediaId: 101, type: 'CRACK', typeLabel: '균열', grade: 'B',
  bboxX: 0.55, bboxY: 0.4, bboxW: 0.1, bboxH: 0.2,
};
const SPALLING = {
  id: 3, mediaId: 103, type: 'SPALLING', typeLabel: '박리박락', grade: 'D',
  bboxX: 0.12, bboxY: 0.6, bboxW: 0.2, bboxH: 0.2,
};

describe('buildReportPdfContext — 하자 박스', () => {
  it('같은 사진의 하자를 한 장으로 묶고 박스를 모두 싣는다', () => {
    const context = buildReportPdfContext(buildReport([CRACK_A, CRACK_B, SPALLING]), null, true);
    const images = context.defectImages ?? [];

    // 하자는 3건이지만 사진은 2장 — 이전에는 3장(101이 2번 중복)이었다.
    expect(images).toHaveLength(2);
    expect(images.map((image) => image.imageUrl)).toEqual(['/media/101/thumb', '/media/103/thumb']);

    expect(images[0].boxes).toEqual([
      { x: 0.32, y: 0.1, width: 0.05, height: 0.71 },
      { x: 0.55, y: 0.4, width: 0.1, height: 0.2 },
    ]);
    expect(images[0].defectCount).toBe(2);
    expect(images[1].boxes).toEqual([{ x: 0.12, y: 0.6, width: 0.2, height: 0.2 }]);
    expect(images[1].defectCount).toBe(1);
  });

  it('bbox가 null이면 박스 없이 사진만 남긴다(수동 추가 하자)', () => {
    const manual = { id: 9, mediaId: 101, type: 'CRACK', typeLabel: '균열', grade: 'A',
      bboxX: null, bboxY: null, bboxW: null, bboxH: null };
    const images = buildReportPdfContext(buildReport([manual]), null, true).defectImages ?? [];
  ...
  it('폭·높이가 0 이하인 좌표는 그리지 않는다', () => {
    ...
    const images = buildReportPdfContext(buildReport([degenerate]), null, true).defectImages ?? [];
