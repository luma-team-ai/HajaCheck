...
// 잔여 정리)는 useLogin.ts와 동일하게 맞춘다 — 공용 PC에서 데모 로그인 전 사용자의 로컬 잔여
// 상태(activeInspectionId/activeReportId·RAG session_id·점검 생성 폼 임시저장 등)가 남아있으면
// useLogin과 같은 크로스유저 노출 문제(#1194, #1590, #1703)가 이 진입점에서도 그대로
// 재현된다 — clearPreviousUserLocalState 하나로 나머지 5개 지점과 동일 계약을 지킨다.
//
