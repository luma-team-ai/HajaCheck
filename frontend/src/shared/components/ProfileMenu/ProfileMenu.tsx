...
export interface ProfileMenuProps {
  /**
   * 소속 기업명 — 개인 회원은 호출부가 "개인 회원" 등 폴백 문구를 넘긴다(mypage ProfileSection과
   * 동일 정책). 미지정 시 기업명·플랜 배지 행 자체를 렌더링하지 않는다(플랫폼 관리자는 기업 소속이
   * 없음 — #773).
   */
  companyName?: string;
  /** 구독 플랜 표기(예: "Free") — mypage PLAN_NAME_LABEL과 동일 소스 사용. companyName 미지정 시 무시됨 */
  planLabel?: string;
  name: string;
  email: string;
  /** 미지정 시 "내 정보" 메뉴 항목 자체를 렌더링하지 않는다(플랫폼 관리자는 마이페이지가 없음 — #773) */
  onMyInfoClick?: () => void;
  /** 미지정 시 "내 플랜" 메뉴 항목 자체를 렌더링하지 않는다(플랫폼 관리자는 구독 대상이 아님 — #773) */
  onMyPlanClick?: () => void;
  onLogout: () => void;
